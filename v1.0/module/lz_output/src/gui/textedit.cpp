...
void TextEdit::initOpenFileName(QString newfilename)
{
    const QStringList args = QCoreApplication::arguments();
    if (args.count() == 2)
        newfilename = args.at(1);
...
void TextEdit::setCurrentFileName(const QString &fileName)
{
    ...
    QString shownName;
    if (fileName.isEmpty())
        shownName = "Print Preview";
    else
        shownName = QFileInfo(fileName).fileName();

    setWindowTitle(tr("%1[*] - %2").arg(shownName).arg(tr("Print Preview")));
