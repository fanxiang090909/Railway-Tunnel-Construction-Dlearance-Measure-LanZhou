...
void ImageViewer3DTwoViewWidget::viewEndImages()
{
    ...
    if (it != currentCheckedModel.rend())
    {
        // 还没有找到起始时找起始帧所在文件
        currentseqno = (*it).seqno;
        current_fc_master = (*it).end_frame_master;
...
bool ImageViewer3DTwoViewWidget::getNasMode()
{
	return isTwoNasMode;
}


void ImageViewer3DTwoViewWidget::exitOuter()
{
    // 清空上一个文件
    if (currentpointsfile != NULL)
    {
...
// 转到帧的相关槽函数
void ImageViewer3DTwoViewWidget::frameSearch()
{
...
void ImageViewer3DTwoViewWidget::frameSearchEditToggled()
{
    if (firsttime)
    {
