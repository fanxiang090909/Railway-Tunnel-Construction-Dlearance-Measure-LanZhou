...
ViewAquizDataWidget::ViewAquizDataWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ViewAquizDataWidget)
{
    ui->setupUi(this);
    ...
    ui->askRawImagesWidget->setEnabled(false);

    // askRawImagesWidget中起始终止帧LineEdit的正则表达式验证
    QRegExp regExp("[0-9]{0,10}");
    // 绑入lineEdit :
    ui->startframe->setValidator(new QRegExpValidator(regExp, this));
    ui->endframe->setValidator(new QRegExpValidator(regExp, this));
    connect(ui->applyimage_Button, SIGNAL(clicked()), this, SLOT(askRawImages()));
    ...
    // 是否为png图显示，默认为false
    ispng = false;
    connect(ui->isjpgbutton, SIGNAL(toggled(bool)), this, SLOT(clickjpgbutton(bool)));
    ui->isjpgbutton->click();
}

ViewAquizDataWidget::~ViewAquizDataWidget()
{
    delete ui;
}

void ViewAquizDataWidget::setParentPath(QString newpath)
{
    tmpopendir = newpath;
}

/*******************************/
void ViewAquizDataWidget::appendMsg(QString msg)
{
    QString currentDate = QTime::currentTime().toString("[hh:mm:ss]");
    ui->statusArea->append(QString("%1%2").arg(currentDate).arg(msg));
}

void ViewAquizDataWidget::appendError(QString msg)
{
    QString currentDate = QTime::currentTime().toString("[hh:mm:ss]");
    ui->statusArea->append(QString("%1%2").arg(currentDate).arg(msg));
}

void ViewAquizDataWidget::openFile()
{
    file_full = QFileDialog::getOpenFileName(this, QObject::tr("打开流式文件"), tmpopendir, "Document files (*.dat *.tmpdat)");
    ...
    else if (filename.endsWith(".tmpdat"))
    {
        isdat = false;
        filename = filename.left(filename.size() - 7);
    }
    ...
    try {
        ...
        ui->askRawImagesWidget->setEnabled(true);
...
void ViewAquizDataWidget::loadFile()
{
...
void ViewAquizDataWidget::depressFile()
{
    ...
    try {
        ...
        if (isdat)
            ret = lzserialimg->readFrameSaveAsImage(startfr, endfr, outputdir.c_str(), true, ispng);
        else
            ret = false;
...
void ViewAquizDataWidget::depressFileByStartEndFC(qint64 tmpstartfc, qint64 tmpendfc)
{
    if (!QFile(file_full).exists())
    {
        QMessageBox::warning(this, tr("提示"), tr("打开文件%1不存在，请重新设置！").arg(file_full));
    ...
    if (tmpstartfc < startfr || tmpendfc > endfr)
    {
        QMessageBox::warning(this, tr("提示"), tr("申请的第%1帧-第%2帧不在文件范围内，请重新设置！").arg(tmpstartfc).arg(tmpendfc));
        return;
    }
    ...
    try {
        ...
        qDebug() << "file frames vector startfc: " << tmpstartfc << ", endfc: " << tmpendfc;
        
        ret = lzserialimg->readFrameSaveAsImage(tmpstartfc, tmpendfc, outputdir.c_str(), true, ispng);
...
void ViewAquizDataWidget::setDepressDir()
{
    // 选择创建目录，默认为刚刚创建的目录
 ...
 * 设置主控fc对应的五组相机fc
 */
bool ViewAquizDataWidget::setCurrentFCs(__int64 master_fc)
{
...
__int64 & ViewAquizDataWidget::getTmpCurrentfc(int j)
{
    return current_fc;
}

bool & ViewAquizDataWidget::getTmphasfc(int j)
{
    switch (j)
    {
 ...
 * 五组原图显示函数
 */
bool ViewAquizDataWidget::updateImages()
{
    ...
    for (int j = -4; j < 5; j++)
    {
        ...
        QString filename3;
        if (ispng)
            filename3 = (QString("%1/%2.png").arg(tmpcamimgdir).arg(tmpcurrentfc + j));
        else
            filename3 = (QString("%1/%2.jpg").arg(tmpcamimgdir).arg(tmpcurrentfc + j));
 ...
 * 起始五帧
 */
void ViewAquizDataWidget::viewBeginFiveImages()
{
    // 已经到最前只能向后申请
    viewdirection = false;
 ...
 * 末尾五帧
 */
void ViewAquizDataWidget::viewEndFiveImages()
{
    // 已经到最后只能向前申请
    viewdirection = true;
 ...
 * 下五帧
 */
void ViewAquizDataWidget::viewNextFiveFrameImages()
{
    // 向后申请
    viewdirection = false;
 ...
 * 上五帧
 */
void ViewAquizDataWidget::viewLastFiveFrameImages()
{
    // 向前申请
    viewdirection = true;
...
// 申请某些帧槽函数
void ViewAquizDataWidget::askRawImages()
{
    __int64 start = ui->startframe->text().toLongLong();
    __int64 end = ui->endframe->text().toLongLong();
    depressFileByStartEndFC(start, end);
}

void ViewAquizDataWidget::clickjpgbutton(bool isjpg)
{
    if (isjpg)
        ispng = false;
    else
...
// 转到帧的相关槽函数
void ViewAquizDataWidget::frameSearch()
{
...
void ViewAquizDataWidget::frameSearchEditToggled()
{
    if (firsttime)
    {
...
// 图像增强处理
void ViewAquizDataWidget::imgEnhance(int newif)
{
    ifimgenhance = newif;
